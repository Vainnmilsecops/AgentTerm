...
export interface QualityGateRunRepository {
  ...
  /**
   * Returns every still-RUNNING run from oldest to newest. Used at desktop
   * startup to reconcile rows whose settlement was never observed by the
   * previous process; empty in steady state.
   */
  listUnsettled(): Promise<readonly QualityGateRun[]>;
...
export type MergeConflictUnavailableReason =
  'GIT_INSPECTION_FAILED' | 'NO_BASE_REF' | 'NOT_HEAD_ATTACHED';
...
export interface WorkflowPluginBindingRepository {
  ...
  findByTaskId(taskId: string): Promise<WorkflowPluginBindingRecord | undefined>;
  /**
   ...
   */
  upsert(record: WorkflowPluginBindingRecord, expectedRevision: number): Promise<void>;

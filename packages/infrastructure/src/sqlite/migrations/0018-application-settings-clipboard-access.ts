/**
 * M13 — store the OSC 52 clipboard read/write toggle alongside Application Settings.
 * The toggle is opt-in: 0 keeps clipboard access disabled (default off).
 *
 * Also relaxes the legacy `schema_version = 1` check constraint to allow the
 * Application Settings schema to evolve. The constraint was enforced with
 * `CHECK (schema_version = 1)` which prevented the rewrite to schemaVersion 2.
 */
export const applicationSettingsClipboardAccessMigration = {
  name: 'application-settings-clipboard-access',
  sql: `
    ...
    INSERT INTO application_settings_new (
      singleton_id, schema_version, revision, default_agent_id, terminal_font_size, mcp_server_token, allow_clipboard_read_write
    )
    SELECT singleton_id, 2, revision, default_agent_id, terminal_font_size, mcp_server_token, 0

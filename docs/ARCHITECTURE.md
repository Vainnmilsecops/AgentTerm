...
This foundation contains runnable desktop/website shells, a pure Domain model for Project and the primary Task lifecycle, Application use cases backed by ports, and Infrastructure adapters for SQLite plus local Git Project discovery, repository snapshots, and Task Worktree lifecycle. Filesystem/Git details remain outside Domain and are reached through Application-owned interfaces. `ProjectDiscovery` owns the narrow open-project workflow; `GitRepositoryInspector` independently returns canonical HEAD, base-branch, and working-tree status data without exposing Git commands. The Task Worktree use cases coordinate deterministic create/reuse/inspect/guarded-cleanup operations with durable SQLite reconciliation checkpoints. Desktop composition, session/runtime lifecycles, terminal rendering, logging pipelines, installers, and product UI are not implemented yet.

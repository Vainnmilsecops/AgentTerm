...
This foundation contains runnable desktop/website shells, a pure Domain model for Project and the primary Task lifecycle, Application use cases backed by ports, and Infrastructure adapters for SQLite plus read-only local Git Project discovery and repository snapshots. Filesystem/Git details remain outside Domain and are reached through Application-owned interfaces. `ProjectDiscovery` owns the narrow open-project workflow; `GitRepositoryInspector` independently returns canonical HEAD, base-branch, and working-tree status data without exposing Git commands. Desktop composition, session/runtime lifecycles, mutating Git/Worktree behavior, terminal rendering, logging pipelines, installers, and product UI are not implemented yet.

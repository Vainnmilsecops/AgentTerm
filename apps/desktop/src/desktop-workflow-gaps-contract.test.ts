...
describe('desktop IPC workflow-gaps contract', () => {
  ...
  it('routes register/unregister through the controller and QualityGateConfiguration form', () => {
    ...
    expect(workspaceSource).toContain('onRegister={onRegisterQualityGate}');
    expect(workspaceSource).toContain('onUnregister={onUnregisterQualityGate}');
    expect(workspaceSource).toContain('controller?.registerQualityGate(input)');
    expect(workspaceSource).toContain('controller?.unregisterQualityGate(gateId)');
  });
});
